...
# --- Local DB Fetch ---
def fetch_local_by_ids(ids):
    return {p.id: p.updated_at.isoformat() for p in Property.objects.filter(id__in=ids)}


# --- Compare External vs Local ---
def compare_updates(external_props):
    external_ids = [item["id"] for item in external_props]
    local_map = fetch_local_by_ids(external_ids)
    ...
    for item in external_props:
        ...
        if not local_updated_at:
            ...
            diffs.append((item, f"Timestamp mismatch: ext={ext_updated_at} | local={local_updated_at}"))

    return diffs


# --- Update/Insert One Property ---
...
if __name__ == "__main__":
    print("🚀 Running autonomous property sync...")
    ext_props = fetch_latest_external()

    if not ext_props:
        print("⚠️ No external properties received. Aborting.")
    else:
        diffs = compare_updates(ext_props)
        ...
        if not diffs:
            ...
            print(f"🔄 {len(diffs)} mismatches found. Updating only those records...")
            for item, reason in diffs:
                print(f"🛠️ Updating ID {item['id']} — Reason: {reason}")
                update_property(item)
